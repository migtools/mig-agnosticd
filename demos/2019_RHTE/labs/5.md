...
4. Next, click to open the `SCCs` section.  You'll see that the `mssql-persistent` application makes use of a custom Security Context Constraint.  e know from our CAM overview, that SCCs are not migrated as part of a Migration Plan.  So, we will need to apply the custom SCC onto our 4.1 Cluster prior to migrating the application. Let's do this now. The custom SCC yaml is available [here](./files/mssql-scc.yaml).  Please download to your local machine, as we will apply it in the next step.
